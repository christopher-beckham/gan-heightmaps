...
def train_discriminator(d, it, steps_per_epoch):
    ...
    return d.fit_generator(it, steps_per_epoch=steps_per_epoch, epochs=1, verbose=False, workers=1, pickle_safe=True)
...
def train_pix2pix(pix2pix, it, steps_per_epoch):
    ...
    return pix2pix.fit_generator(it, steps_per_epoch=steps_per_epoch, epochs=1, verbose=False, workers=1, pickle_safe=True)
...
def evaluate(models, generators, losses, params):
    ...
    # Evaluate
    d_loss = d.evaluate_generator(d_gen, steps=params.val_samples // params.batch_size)
    p2p_loss = p2p.evaluate_generator(p2p_gen, steps=params.val_samples // params.batch_size)
...
if __name__ == '__main__':
    ...
    #for model in models.values():
        ...
        #model.predict = threadsafe_fn( model.predict, lock )
        #model.train = threadsafe_fn( model.predict, lock )
        #model.train_on_batch = threadsafe_fn( model.predict, lock )

    #import pdb
    #pdb.set_trace()
        
    train(models, it_train, it_val, params)
    #debugtest(models, it_train, it_val, params)
